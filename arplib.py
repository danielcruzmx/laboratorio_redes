...
def restore_network(v1_ip, v1_mac, v2_ip, v2_mac):
    send(ARP(op=2, hwdst="ff:ff:ff:ff:ff:ff", pdst=v1_ip, \
                   hwsrc=v2_mac, psrc=v2_ip),count=5)
    send(ARP(op=2, hwdst="ff:ff:ff:ff:ff:ff", pdst=v2_ip, \
                   hwsrc=v1_mac, psrc=v1_ip),count=5)
