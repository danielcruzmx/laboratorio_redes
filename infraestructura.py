...
class imagen:
    ...
    def _list(self):
        print " "
        print "{:15} {:30} {:20}".format('ID','TAG','SIZE MB')
        for c in self.client.images.list():
            if c.attrs['RepoTags']:
                print  "{:15} {:30} {:20}".format(c.attrs['Id'].split(':')[1][:12], c.attrs['RepoTags'][0], str(c.attrs['Size']/1000000)[:20])
            
    def _create(self, dockerfile, tag):
        parent_dir = os.path.dirname(os.path.realpath(__file__))
        response = [line for line in self.client.images.build(path = parent_dir, dockerfile=dockerfile, rm=True, tag=tag)]
        for r in response[1]:
...
class contenedor:
    ...
    def __init__(self, item):
        self.client = docker.from_env(version = "auto")
        self.nombre = item['name']
        #self.c = self.__exists(item['name'])
        self.item = item
    ...
    def remove(self):
        ...
        if self.c:
            ...
            print ""
            print "Contenedor", self.c.attrs['Name'].split('/')[1] ,"removido"
            print ""
    ...
    def info(self):
        ...
        if self.c:
            print " "
            print "{:15} {:20} {:15} {:30}".format('ID','NOMBRE','ESTADO','IMAGEN')
